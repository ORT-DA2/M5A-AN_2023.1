...
namespace Uyflix.WebApi.Test
{
    ...
    public class MoviesControllerTest
    {
        ...
        public void GetMoviesOk()
        {
            ...
            var body = objectResult.Value as IEnumerable<Movie>;

            mock.VerifyAll();
            Assert.AreEqual(200, statusCode);
            Assert.IsTrue(body.SequenceEqual(movies));
        }
        ...
        public void PostMovieOk()
        {
            ...
            var objectResult = result as ObjectResult;
            var statusCode = objectResult.StatusCode;
            var body = objectResult.Value as Movie;

            mock.VerifyAll();
            Assert.AreEqual(200, statusCode);
            Assert.IsTrue(iceAge.Equals(body));
        }
        ...
        {
            var iceAgeModified = iceAge;
            iceAgeModified.Name = "Test";
            mock.Setup(x => x.UpdateMovie(iceAgeModified)).Returns(iceAgeModified);
            var result = api.PutMovie(iceAge.Id, iceAgeModified);
            var objectResult = result as ObjectResult;
            var statusCode = objectResult.StatusCode;
            var body = objectResult.Value as Movie;
            ...
            mock.VerifyAll();
            Assert.AreEqual(200, statusCode);
            Assert.IsTrue(iceAgeModified.Name.Equals(body.Name));
        }
